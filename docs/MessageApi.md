...
[**getMessagesOfSender**](MessageApi.md#getMessagesOfSender) | **GET** /messages | Lista de mensajes del remitente dado
...
> MessageGetResponse2 getMessagesOfSender(senderId, opts)
...
apiInstance.getMessagesOfSender(senderId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
